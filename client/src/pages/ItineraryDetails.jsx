    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
      {details.hotels.map((hotel, index) => {
       // const hotelDescription = hotel.desc || hotel.description || "Exclusive luxury suite curated for your comfort.";
        ...
        //const features = hotelDescription.split(',').map(item => item.trim());
          ...
          <div key={index} className="bg-[#111625] rounded-[1.5rem] overflow-hidden border border-slate-800/80 shadow-xl group flex flex-col justify-between hover:border-emerald-500/20 transition-all duration-300">
            <div>
              ...
              <div className="p-5">
                ...
                <div className="flex items-center gap-2 mb-4 bg-[#070a13]/40 w-fit px-2.5 py-0.5 rounded-md border border-slate-800/60">
...
<div className="space-y-1.5 border-t border-slate-800/50 pt-3">
  {(hotel.desc || hotel.description || "Exclusive luxury suite curated for your comfort.")
    .split('.')
    .map(sentence => sentence.trim())
    .filter(sentence => sentence.length > 0) // හිස් වාක්‍ය අයින් කරනවා
    .slice(0, 2) // Card එක ඇතුලේ ඉඩ මදි නිසා මුල් වාක්‍ය 2 විතරක් පෙන්වනවා
    .map((sentence, idx) => (
    ...
    <div className="bg-[#111625] w-full max-w-4xl rounded-[2rem] overflow-hidden shadow-2xl border border-slate-800 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2">
        ...
        <div className="p-8 md:p-10 space-y-5 overflow-y-auto max-h-[75vh] md:max-h-[85vh] text-left">
         ...
         {/* MODAL DETAILED AMENITIES WITH ICONS */}
<div className="bg-[#070a13]/40 p-5 rounded-xl border border-slate-800/60 space-y-3">
  <p className="text-slate-400 text-[10px] uppercase font-bold tracking-wider mb-1 text-emerald-400">Highlights & Details</p>
  <div className="space-y-3.5">
    {(selectedHotel.desc || selectedHotel.description || "Exclusive luxury suite curated for your comfort.")
      .split('.')
      .map(sentence => sentence.trim())
      .filter(sentence => sentence.length > 0) // හිස් වාක්‍ය අයින් කරනවා
      .map((sentence, idx) => (
        <div key={idx} className="flex items-start gap-3 text-slate-300 text-xs md:text-sm font-light leading-relaxed border-b border-slate-800/30 pb-2 last:border-none last:pb-0">
          
          <span className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)] shrink-0 mt-1.5" />
          <p>{sentence}.</p>
        </div>
      ))}
  </div>
